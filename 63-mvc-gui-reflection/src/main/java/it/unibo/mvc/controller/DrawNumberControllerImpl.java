...
public final class DrawNumberControllerImpl implements DrawNumberController {
    ...
    public void newAttempt(final int n) {
        if (views.isEmpty()) {
            throw new IllegalAccessError("The views collection is empty!");
        }
        for (DrawNumberView elem : views) {
        Objects.requireNonNull(elem, "There is no view attached!").result(model.attempt(n));
        }
    }
